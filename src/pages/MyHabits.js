...
export const MyHabits = () => {
  ...
  useEffect(()=>{
    const showDaily = ()=>{
      ...
      if(analyses.length === 0 ){
        show = false
      }else if(analyses[analyses.length - 1]?.updatedAt || analyses.habits.length === 0 ){
       ...
       const date = updatedLast.getDate()
        ...
        if((date + 1 === now )|| updatedLast.getHours() === createdAt.getHours() ){
           show = true
        }else{
           show = false
        }
  ...
  return (
    <PositionContainer style={{margin: '50px 0'}} >
      ...
      {showButtonAddDeleteHabitAndAddAnalysis()? <ButtonDefault onClick={addAnalysis}>
        ...
        analyses[analyses.length - 1]?.duration > 0 ? <h1>You have completed only {analyses[analyses.length - 1]?.duration}{analyses[analyses.length - 1]?.duration === 1 ? 'day': 'days'}</h1> : <h1>You can do anything!</h1>
      }
      {showComponentDaily === false && <Habit habits={habits} getHabit={getHabit} condition={showButtonAddDeleteHabitAndAddAnalysis} />}
